// Frozen PyTorch FP32-B up/down/noninteger antialias inputs and outputs.
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        double maximum = 0.;
        for (const auto &sizes : {std::array<int, 2>{64, 128}, {128, 64}, {96, 128}, {160, 128},
                                  {257, 128}, {192, 64}, {64, 80}, {129, 128}}) {
            const int source = sizes[0], target = sizes[1];
            std::size_t case_failures = 0; double case_maximum = 0.;
            ...
            for (int c = 0; c < 3; ++c) {
                ...
                for (int i = 0; i < target * target; ++i) {
                    ...
                    ++checked;
                    const bool failed = !std::isfinite(plane[i]) || delta > 2e-7;
                    if (failed && case_failures < 4)
                        std::cerr << name << " c=" << c << " y=" << i/target << " x=" << i%target
                                  << " expected=" << std::hexfloat << reference << " actual=" << plane[i]
                                  << std::defaultfloat << " delta=" << delta << '\n';
                    failures += failed; case_failures += failed;
                    case_maximum = std::max(case_maximum, delta);
                }
            }
            std::cout << name << " max_abs=" << case_maximum << " violations=" << case_failures << '\n';
        ...
        return checked == 289536 && failures == 0 ? 0 : 1;
