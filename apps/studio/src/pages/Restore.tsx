    ...
    <div className="restoration-layout">
      ...
      <aside className="panel restore-controls"><div className="panel-heading"><h2>处理设置</h2><Tag color={ready ? 'cyan' : 'default'}>SeedVR2 · 3B</Tag></div>
        <Form layout="vertical" onFinish={start}>
          ...
          {video && <><Form.Item label="测试片段"><Select aria-label="测试片段帧数" value={frames} disabled={active} onChange={setFrames} options={[5,9,17].map(v => ({ value: v, label: `开头 ${v} 帧${picture?.fps ? ` · 约 ${(v/picture.fps).toFixed(2)} 秒` : ''}` }))}/></Form.Item><p className="field-note">8 位 SDR 短片预览：最多 17 帧、128 像素长边，输出 MP4，不含音轨。输入不足时按实际帧数输出。</p><p className="field-note">实验性功能：部分视频测试的数值一致性尚未通过。请先用短片检查结果，详见<Link to="/models">模型验收</Link>。</p></>}
