...
def main():
    ...
    with (ROOT/'.cache/bounded-video-validation.lock').open('w') as lock:
        ...
        native = frozen/'install/bin'/a.binary.name
        native.parent.mkdir(parents=True)
        shutil.copy2(a.binary, native)
        prefix = a.binary.absolute().parent.parent
        for folder in ('lib', 'lib64'):
            if (prefix/folder).is_dir():
                shutil.copytree(prefix/folder, frozen/'install'/folder, symlinks=True)
