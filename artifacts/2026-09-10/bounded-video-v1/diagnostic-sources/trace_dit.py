...
def main():
    ...
    height, width = baseline['output']['height'], baseline['output']['width']
    grid = [baseline['clip']['latent_frames'], height//16, width//16]
    ...
    snapshot.mkdir()
    dynamic = subprocess.check_output(['readelf', '-d', str(args.binary.resolve())], text=True, timeout=10)
    sdk_names = re.findall(r'Shared library: \[(libseedvr2\.so[^\]]*)\]', dynamic)
    if len(sdk_names) != 1 or Path(sdk_names[0]).name != sdk_names[0]:
        raise ValueError('Expected exactly one named SeedVR2 SDK dependency in the Linux executable')
    binary, sdk = snapshot/'seedvr2', snapshot/sdk_names[0]
    ...
    for index in range(args.first, args.last+1):
        ...
        inputs = [copy_tensor(root, item, f'input-{i}.f32', shape) for i, ((root, item), shape)
                  in enumerate(zip(sources, ([*grid, 2560], [58, 2560])))]
        ...
        if completed.returncode:
            raise RuntimeError(f'Native block exited {completed.returncode}; see {folder / "stderr.log"}')
        run = load_json(folder/'stdout.json')
        if (completed.returncode or run['status'] != 'EXECUTED' or
                any(run['implementation'].get(key) != value for key, value in implementation.items())):
        ...
        row = dict(index=index, input_kind=input_kind, case_sha256=sha(case_path),
                   run_sha256=sha(folder/'stdout.json'), stderr_sha256=sha(folder/'stderr.log'),
                   reported_implementation=run['implementation'], metrics=metrics)
