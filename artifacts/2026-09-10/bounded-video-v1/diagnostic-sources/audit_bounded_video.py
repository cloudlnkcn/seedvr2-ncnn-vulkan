...
def main():
    ...
    sdk_hashes = {digest for path, digest in plan['files'].items()
                  if path.startswith(('install/lib/', 'install/lib64/'))
                  and Path(path).name.startswith('libseedvr2.so')}
    if not sdk_hashes:
        raise ValueError('Frozen installed SDK identity is missing')
    ...
    for case in fixture['cases']:
        ...
        if (run['implementation']['executable_sha256'] != plan['files']['install/bin/seedvr2'] or
                run['implementation']['sdk_library_sha256'] not in sdk_hashes):
        ...
        result.append(dict(case_id=name, source_and_tensor_audit_passed=True,
            ...
            compared_elements=sum(math.prod(row['shape']) for row in rows), stages=rows))
