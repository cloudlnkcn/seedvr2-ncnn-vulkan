...
int main(int argc,char **argv) {
    ...
    try {
        ...
        if(name=="patch-in") {
            ...
            const bool reduction=name=="reduction" || name=="reduction-local";
            if(!reduction && name!="erf" && name!="celu") throw std::runtime_error("Unknown case");
            const int n=reduction?65536:1024;ncnn::Mat m(n);
            double sum=0;
            for(int i=0;i<n;++i) {
                m[i]=reduction?float((i%31)-15)/32.f+1.f/256.f:float(i-512)/128.f;
                sum+=m[i];
        ...
        const bool shape_ok=name=="patch-in"?(result.dims==2 && result.w==2560 && result.h==320):
            ...
            (result.dims==1 && result.w==((name=="reduction" || name=="reduction-local")?1:1024));
