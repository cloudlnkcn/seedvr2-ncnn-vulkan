...
ncnn::Mat prepare_image(const ImagePixels &input, int long_side) {
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        for (int y = 0; y < input.height; ++y)
            for (int x = 0; x < rw; ++x) {
                float value = 0;
                // Keep one rounding per accumulation, matching the pinned
                // PyTorch antialias kernel independently of compiler FMA flags.
                for (std::size_t j = 0; j < fx[x].weights.size(); ++j)
                    value = std::fma(input.rgb[(std::size_t(y)*input.width+fx[x].first+j)*3+c]/255.f,
                                     fx[x].weights[j], value);
                temp.row(y)[x] = value;
            }
        ...
        for (int y = 0; y < height; ++y)
            for (int x = 0; x < width; ++x) {
                ...
                for (std::size_t j = 0; j < filter.weights.size(); ++j)
                    value = std::fma(temp.row(filter.first+int(j))[x+ox], filter.weights[j], value);
