...
inline std::vector<ncnn::Mat> execute_graph(const GraphFiles &files, const std::string &id,
    ...
    // Measured accuracy policy for the temporal encoder. SGEMM rounding in
    // its flattened 3D convolutions is amplified by the subsequent DiT blocks.
    // The decoder does not feed that trajectory and keeps the faster default.
    const bool direct_encoder = !request.vulkan && id == "encoder" && inputs.at(0).dims == 4;
    if (direct_encoder) net.opt.use_sgemm_convolution = false;
