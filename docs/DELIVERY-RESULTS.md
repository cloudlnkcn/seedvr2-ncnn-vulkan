...
**同日后续更新：17 帧的原始数值失败已通过针对性修复解决；最终同一份 SDK 的 CPU/Vulkan 都通过 73/73。** 相位表、Q/K 预缩放和 CPU 编码器策略的根因证据、负候选、最终回归与实测成本见 [VIDEO-NUMERICS.md](VIDEO-NUMERICS.md)。以下表格与 SHA 保留早先交付批次的真实历史身份，不能当作修复后程序的状态。
...
同日新增的独立测试见 [FP16/BF16 实测](PRECISION-VALIDATION.md)：真实复现了最新 ncnn 的 BF16 reduction 修复，并用官方 17 帧轨迹中的真实 patch-in 对照六种精度。该组件的 FP16 存储/FP32 运算有 3/819,200 个元素超出既有 FP32 预算；完整低精度模型未执行，应用精度和上述交付记录保持各自的原始身份。
