...
本机 NVIDIA 的原生套件为 **36/36**。数值修复版 `7e56479c0c1069ee6b6c3ff88f8d57af12955418` 的 Ubuntu GCC CLI、Clang CLI、GCC Web 三项远端任务也全部通过，每项原生套件为 **24 项通过、12 项能力跳过**；显式 Mesa 子集为 **10 项通过、12 项能力跳过**，不能与原生套件相加。Web 另有 **54/54** 接口检查。远端没有下载完整 3B 权重；六条全链数值回归使用上面的本机实测记录。[远端原始报告与依赖日志](../artifacts/2026-09-10/github-ci-numerics-v2/README.md)。
